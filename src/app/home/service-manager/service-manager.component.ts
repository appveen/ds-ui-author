...
export class ServiceManagerComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        self.subscriptions['entity.delete'] = self.commonService.entity.delete.subscribe(data => {
        ...
        self.subscriptions['entity.status'] = self.commonService.entity.status.subscribe(data => {
            const index = self.serviceList.findIndex((s) => {
                if (s._id === data._id) {
                    return s;
                }
        ...
        self.subscriptions['entity.new'] = self.commonService.entity.new.subscribe(data => {
            const index = self.serviceList.findIndex((s) => {
                if (s._id === data._id) {
                    return s;
                }
    ...
    newService() {
        const self = this;
        self.newServiceModalRef = self.commonService.modal(self.newServiceModal, { size: 'sm' });
        self.newServiceModalRef.result.then(close => {
    ...
    discardDraft(id: string) {
        ...
        self.alertModalTemplateRef = self.commonService.modal(self.alertModalTemplate, { size: 'sm' });
        self.alertModalTemplateRef.result.then(close => {
            if (close) {
                ...
                request.subscribe(res => {
                    ...
                    if (service.status !== 'Draft') {
                        self.router.navigate(['/app/', self.commonService.app._id, 'sb', id]);
                    } else {
                        if (serviceIndex > -1) {
    ...
    triggerServiceCreate() {
        ...
        self.commonService.post('serviceManager', `/${this.commonService.app._id}/service`, payload).subscribe(res => {
            ...
            self.router.navigate(['/app/', self.commonService.app._id, 'sb', res._id]);
        }, err => {
            self.showLazyLoader = false;
            self.commonService.errorToast(err);
        });
    ...
    editService(index: number) {
        const self = this;
        if ((self.serviceList[index].status === 'Draft' || self.serviceList[index].draftVersion) && !self.showCardDraft[index]) {
            self.showCardDraft[index] = true;
            return;
        }
        self.appService.editServiceId = self.serviceList[index]._id;
        self.router.navigate(['/app/', self.commonService.app._id, 'sb', self.appService.editServiceId]);
    }
    ...
    cloneService(index: number) {
        ...
        self.cloneServiceModalRef = self.commonService.modal(self.cloneServiceModal, { size: 'sm' });
        self.cloneServiceModalRef.result.then(close => {
            self.cloneData = null;
            ...
            .subscribe(res => {
                ...
                if (res.length > 0) {
                    ...
                    res.forEach((service, i) => {
                        ...
                        service.docapi = `${environment.url.doc}/?q=/api/a/sm/${service.app}/service/utils/${service._id}/swagger`;
                        service._records = 0;
                        self.serviceList.push(service);
                        // }
                    });
                    ...
                    self._getAllServiceRecords(res.map(e => e._id)).subscribe((counts: any) => {
                        ...
                        self.commonService.errorToast(err, 'We are unable to fetch Data Service documents count, please try again later');
                    });
                }
            }, err => {
    ...
    deleteService(index) {
        ...
        self.alertModal.message = 'Are you sure you want to delete this data service? This action will delete '
    ...
    toggleServiceStatus(index: number) {
        ...
        if (self.serviceList[index].status === 'Active') {
            ...
            self.alertModal.message = 'Are you sure you want to stop this data service? Once stopped, "'
                + self.serviceList[index].name
            ...
            self.alertModal.message = 'Are you sure you want to start this data service? Once started, "'
                + self.serviceList[index].name
    ...
    deployService(index: number) {
        const self = this;
        self.alertModal.statusChange = true;
        if (self.serviceList[index].status === 'Draft' || self.serviceList[index].draftVersion) {
            self.alertModal.title = 'Deploy ' + self.serviceList[index].name + '?';
            self.alertModal.message = 'Are you sure you want to Deploy this data service? Once Deployed, "'
                + self.serviceList[index].name
