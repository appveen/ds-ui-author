...
@Component({
  selector: 'odp-b2b-flow-cell',
  templateUrl: './b2b-flow-cell.component.html',
  styleUrls: ['./b2b-flow-cell.component.scss']
})
export class B2bFlowCellComponent implements ICellRendererAngularComp {
  ...
  agInit(params: ICellRendererParams): void {
    ...
    if (params.data && this.field === 'url') {
      this.value = params.data.inputStage && params.data.inputStage.options ? params.data.inputStage.options.path : '';
      this.value = '/b2b/' + this.params.data.app + this.value;
    }
  }
  ...
  redirectToManage(edit?: boolean) {
    ...
    this.router.navigate(['/app', this.commonService.app._id, 'flow', this.data?._id]);
  }
  ...
  stopFaas(data) {
    ...
    let request;
    request = this.commonService.put('partnerManager', `/${this.commonService.app._id}/flow/utils/${data._id}/stop`, data);
  ...
  startFaas(data) {
    ...
    data.status = 'Pending';
    let request;
    request = this.commonService.put('partnerManager', `/${this.commonService.app._id}/flow/utils/${data._id}/start`, data);
    request.subscribe(res => {
  ...
  closeDeleteModal(data) {
    if (data) {
      ...
      this.commonService
        .delete('partnerManager', `/${this.commonService.app._id}/flow/${this.data._id}`).subscribe(_d => {
