...
@Component({
    selector: 'odp-post-hook-logs',
    templateUrl: './post-hook-logs.component.html',
    styleUrls: ['./post-hook-logs.component.scss']
})
export class PostHookLogsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        self.getCount(true);
        if (self.logsService.postHookFilter) {
            self.search(self.logsService.postHookFilter);
            self.logsService.postHookFilter = null;
        } else {
            self.clearSearch();
        }
    ...
    getRecords() {
        ...
        self.subscriptions['getlogs'] = self.commonService.get('log', `/mon/appCenter/${self.service._id}/postHook`, self.apiConfig)
            .subscribe(d => {
    ...
    getCount(firstLoad?: boolean) {
        ...
        self.subscriptions[firstLoad ? 'getlogsCountFirst' : 'getlogsCount'] =
            self.commonService.get('log', `/mon/appCenter/${self.service._id}/postHook/count`, options).subscribe(count => {
                self.showLazyLoader = false;
    ...
    showObject(index) {
        ...
        self.postlogs.push(self.logs[index].logs);
