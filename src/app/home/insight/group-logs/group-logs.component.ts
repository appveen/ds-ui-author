...
import { _, GridApi, GridOptions, IDatasource, IGetRowsParams } from 'ag-grid-community';
...
export class GroupLogsComponent implements OnInit, OnDestroy {
      ...
      getRows: (params: IGetRowsParams) => {
        ...
        self.currentRecordsCountPromise.then(count => {
          if (params.endRow - 30 < self.currentRecordsCount) {
            ...
            self.subscriptions['getRecords_' + self.apiConfig.page] = self.commonService.get('log', `/mon/author/group/log`, self.apiConfig)
              .subscribe(rows => {
  ...
  getRecordsCount(first?: boolean) {
    ...
    self.currentRecordsCountPromise = self.commonService.get('log', '/mon/author/group/log' + '/count', { filter: filter, noApp: true }).pipe(
      map(count => {
        if (first) {
