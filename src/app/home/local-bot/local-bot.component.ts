...
export class LocalBotComponent implements OnInit {
  @ViewChild('newBotTextarea', { static: false }) newBotTextarea: HTMLInputElement;
  ...
  createBot() {
    const self = this;
    self.isCreate = true;
    self.newBotModalRef = self.commonService.modal(self.newBotModal, { size: 'sm' });
  ...
  editBot() {
    ...
    self.editBotModalRef = self.commonService.modal(self.newBotModal, { size: 'sm' });
    self.editBotModalRef.result.then(close => {
      if (close) {
  ...
  createBotKey() {
    const self = this;
    self.isCreate = true;
    self.newKeyModalRef = self.commonService.modal(self.newKeyModal, { size: 'sm' });
    self.newKeyModalRef.result.then(close => {
      if (close) {
  ...
  deleteUser(id?: string) {
    ...
    alertModal.message = 'Are you sure you want to delete <span class="text-delete font-weight-bold">' + self.selectedBot.basicDetails.name
      + '</span> bot?';
